/*
  Copyright 2017 Amaury ABRIAL <yruama_lairba at hotmail dot com>
  Copyright 2021 Yann COLLETTE <ycollette dot nospam at free dot fr>
...
#define ECHO_URI "https://github.com/ycollet/smoothed-echo"
...
#define MAX_DELAY_IN_SAMPLE (2 * 44100)  // 2 sec at 44100hz
...
typedef struct {
  ...
  const float * smooth_delay;
  const float * smooth_feedback;
  const float * input;
  float *       output;
  float * delay_buffer;
...
static LV2_Handle instantiate(const LV2_Descriptor*     descriptor,
  ...
  echo->delay_buffer = (float *)calloc(echo->delay_buffer_size, sizeof(float));
...
static void connect_port(LV2_Handle instance,
  ...
  case ECHO_FEEDBACK:
    ...
    break;
  case ECHO_SMOOTH_DELAY:
    echo->smooth_delay = (const float *)data;
    break;
  case ECHO_SMOOTH_FEEDBACK:
    echo->smooth_feedback = (const float *)data;
    break;
  case ECHO_INPUT:
...
static void run(LV2_Handle instance, uint32_t n_samples) {
  ...
  const float         smooth_delay      = *(echo->smooth_delay);
  const float         smooth_feedback   = *(echo->smooth_feedback);
  const float         old_delay         = echo->old_delay;
  const float         old_feedback      = echo->old_feedback;
  ...
  // Use a smoother for feedback and delay parameters
  // Because if automated by a midi knob, because
  // the know varies between 0 and 256 you can here
  // some noise glitches while changing the knob position

  float new_delay    = old_delay    + smooth_delay    * (delay    - old_delay);
  float new_feedback = old_feedback + smooth_feedback * (feedback - old_feedback);
  ...
  for (uint32_t pos = 0; pos < n_samples; pos++) {
    ...
    float output_sample = input_sample + (new_feedback * delay_sample);
  ...
  echo->old_delay    = new_delay;
  echo->old_feedback = new_feedback;
}
